...
const emoji_1 = require("../data/emoji");
const db_1 = require("../db");
class ReactionModel {
    static async initializeTables() {
        const client = await (0, db_1.createClientAndConnect)();
        if (!client) {
        ...
        try {
            ...
            `);
            for (const emoji of emoji_1.DEFAULT_EMOJIS) {
                await client.query(`
    ...
    static async getReactions(topicId) {
        const client = await (0, db_1.createClientAndConnect)();
        if (!client) {
            throw new Error('Failed to connect to database');
        }
        try {
            ...
            `, [topicId, 1]); // TODO: Заменить на реального пользователя
            return result.rows;
        }
        catch (error) {
    ...
    static async toggleReaction(topicId, userId, emojiCode) {
        const client = await (0, db_1.createClientAndConnect)();
        if (!client) {
            throw new Error('Failed to connect to database');
        }
        try {
            ...
            const existingReaction = await client.query(`SELECT id FROM reactions 
                WHERE topic_id = $1 AND user_id = $2 AND emoji_code = $3`, [topicId, userId, emojiCode]);
            let isAdded = false;
            if (existingReaction.rows.length > 0) {
                ...
                await client.query(`INSERT INTO reactions (topic_id, user_id, emoji_code)
                    VALUES ($1, $2, $3)`, [topicId, userId, emojiCode]);
                isAdded = true;
            }
            const countResult = await client.query(`SELECT COUNT(*) as count
                FROM reactions
                WHERE topic_id = $1 AND emoji_code = $2`, [topicId, emojiCode]);
